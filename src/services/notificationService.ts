...
function SessionCard({ session, settings, onAddTime, onOpenConsumption, onOpenSummary, onOpenHistory, onSessionClosed, isHighlighted = false }: SessionCardProps) {
  ...
  useEffect(() => {
    // Check if time is up AND a notification has not already been sent for this session.
    // `timeUpNotificationSent` is persisted in Firestore, solving the re-render issue.
    if (timerState.isTimeUp && !session.timeUpNotificationSent) {
      (async () => {
        try {
          // This service function now handles creating the notification
          // AND updating the session in one atomic operation.
    ...
    // Dependency on `session` ensures we have the latest `timeUpNotificationSent` value.
  }, [timerState.isTimeUp, session]);
