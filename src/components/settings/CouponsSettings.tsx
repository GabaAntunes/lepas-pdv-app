...
export function CouponsSettings() {
  ...
  return (
    <Card>
      <CardHeader>
        <BlurFade delay={0.15}>
          <CardTitle className="font-headline">Cupons de Desconto</CardTitle>
        </BlurFade>
        <BlurFade delay={0.25}>
            ...
            <TableBody>
              {loading ? (
                ...
                filteredCoupons.map((coupon, idx) => (
                  <BlurFade as="tr" key={coupon.id} delay={0.45 + idx * 0.05}>
                    <TableRow>
                      <TableCell className="font-medium font-code">{coupon.code}</TableCell>
                      <TableCell>{formatDiscount(coupon)}</TableCell>
                      <TableCell>
                        <span className={`px-2 py-1 text-xs rounded-full ${coupon.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300'}`}>
